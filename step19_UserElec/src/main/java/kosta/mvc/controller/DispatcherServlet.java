...
public class DispatcherServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(key==null||key.equals("")) {
			key="elec"; //key공백일때 elec
		}
		if(methodName==null||methodName.equals("")) {
			methodName="select"; //method공백일때 select
		}
		...
		if(mv.isRedirect()) {
			...
			e.printStackTrace();
			request.setAttribute("errorMsg", e.getCause().getMessage());//원인 정확한 메세지 나오게넣음
			request.getRequestDispatcher("error/error.jsp").forward(request, response);//에러났을때 보내기
		}
	}//service끝
